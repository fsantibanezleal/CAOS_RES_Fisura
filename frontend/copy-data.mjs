...
// replays them, and inline the fisuralab sources for the live (Pyodide) lane. Canonical copies live in ../data
...
// 2) inline the fisuralab Python sources for the optional Pyodide live lane -> public/pyodide/sources.json
const pkg = join(ROOT, 'data-pipeline', 'fisuralab');
if (existsSync(pkg)) {
  const sources = {};
  const walk = (dir, rel = '') => {
    for (const e of readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (e.name.endsWith('.py')) sources[`fisuralab/${r}`] = readFileSync(abs, 'utf-8');
    }
  };
  walk(pkg);
