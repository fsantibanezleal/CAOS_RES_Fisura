...
`data-pipeline/fisuralab/core/gate.py :: classify_lane()`. A case runs **live** in the browser (Pyodide) iff , 
...
disagrees with the gate, so a heavy model can never be mislabeled "live". The classical cases are pure-Python +
