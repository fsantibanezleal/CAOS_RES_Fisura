...
`data-pipeline/fisuralab/pipeline.py` orchestrates the **named stages** (frozen names/signatures, rework bodies):
...
| infer | `stages/infer.py` | run the ladder L0-L5 over every sample |
...
Run: `python -m fisuralab.pipeline [all|<case_id>] [--seed N]` (or `scripts/precompute.{sh,ps1}`). It writes
