...
`data-pipeline/fisuralab/io/contract.py`. Declares the required schema (columns, units, ranges) + an explicit
...
`data-pipeline/fisuralab/core/{trace.py, manifest.py}`. Every run writes a compact trace (`example.trace/v1`) +
