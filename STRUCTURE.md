...
> generic names. In the tree actually shipped: **`productlab/` = `data-pipeline/fisuralab/`** (the Python
> package lives INSIDE `data-pipeline/`; on instantiation rename `fisuralab` → `<slug>lab`), **`web/` =
